...
java -Xms1g -Xmx4g -cp ~/workspace/baking/h2r-baking.jar edu.brown.cs.h2r.baking.Experiments.ManyAgentsSchedulingRealDataHeldOut $SAVE_FILE ${SGE_TASK_ID:-1} dessert robots shelf    2>>"$ERR_DIRECTORY/${4:-$SGE_TASK_ID}.err" 1>> "$DIRECTORY/${2:-$SGE_TASK_ID}.csv"
