...
public class AgentHelper {
	...
	public static List<KitchenSubdomain> generateAllRTDFPolicies(Domain generalDomain, State startingState, List<Recipe> recipes, RewardFunction rf, StateHashFactory hashingFactory) {
		...
		for (int i = 0; i < recipes.size(); i++) {
			...
			policyDomains.addAll(AgentHelper.generateRTDPPolicies(recipe, domain, startingState, rf, hashingFactory));
		}
	...
	public static List<KitchenSubdomain> generateRTDPPolicies(Recipe recipe, Domain domain, State state, RewardFunction rf, StateHashFactory hashingFactory) {
		...
		do {
			...
			for (BakingSubgoal sg : activeSubgoals) {
				...
				KitchenSubdomain subdomain = AgentHelper.generateRTDFPolicy(domain, state, recipe, sg, rf, hashingFactory);
	...
	public static KitchenSubdomain generateRTDFPolicy(Domain domain, State startingState, Recipe recipe, BakingSubgoal subgoal, RewardFunction rf, StateHashFactory hashingFactory)
	{
	...
	public static void setSubgoal(KitchenSubdomain policyDomain) {
