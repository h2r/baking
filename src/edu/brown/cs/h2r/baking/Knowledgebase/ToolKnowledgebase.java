	...
	public void addTools(Domain domain, State state, String space) {
		for (Entry<String, BakingInformation> entry : this.toolMap.entrySet()) {
			...
			try {
				...
				transportable = info.getBoolean(BakingInformation.toolTransportable);
			} catch (BakingCastException e) {
				e.printStackTrace();
			}
			
			if (transportable) {
				state.addObject(ToolFactory.getNewCarryingToolObjectInstance(domain, name, toolTrait, toolAttribute, space));
			} else {
